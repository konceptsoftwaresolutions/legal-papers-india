...
import {
  getServiceCategoryStatusLeadsDataTotalLeadsThunkMiddleware,
  getServiceCategoryStatusLeadsDataTotalThunkMiddleware,
} from "../../redux/features/bucket";
import { TiArrowForwardOutline } from "react-icons/ti";

const TotalLeadsBucketView = () => {
  ...
  const { serviceCategoryStatusLeadsDataTotalLeads } = useSelector(
    (state) => state.bucket
  );

  const allLeads = useMemo(() => {
    return serviceCategoryStatusLeadsDataTotalLeads?.allLead;
  }, [serviceCategoryStatusLeadsDataTotalLeads]);

  const totalPages = useMemo(() => {
    // return serviceCategoryStatusLeadsData?.countAllLead;
    const countAllLead = serviceCategoryStatusLeadsDataTotalLeads?.countAllLead;
    return countAllLead ? Math.ceil(countAllLead / 10) : 500;
  }, [serviceCategoryStatusLeadsDataTotalLeads]);

  const pageNumber = useMemo(() => {
    const query = path.searchQuary?.[0];
    return query?.page && query?.page !== "" ? parseInt(query?.page) : 1;
  }, [path.location]);
  ...
  useEffect(() => {
    ...
    console.log(payload);
    dispatch(
      getServiceCategoryStatusLeadsDataTotalLeadsThunkMiddleware(payload)
    );
  ...
  const handlePageChange = (page) => {
    ...
    // setCurrentPage(page);
    path.changeEndPoint(
      `nc-bucket-share-by-selecting?page=${page}&serviceCategory=${payload?.serviceCategory}&status=${payload?.status}`
    );
  };
  ...
  return (
    <>
      <LeadFilter
        ...
        filterFor="total-leads-bucket-view"
        endPoint="total-leads-bucket-view"
      ...
      <div className="flex flex-col w-full px-4 gap-y-4 py-5">
        <div className="grid grid-cols-2">
          <Heading text="Total NC Bucket View" showHeading />
        </div>
...
export default TotalLeadsBucketView;
