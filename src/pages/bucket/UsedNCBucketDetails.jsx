...
import {
  ...
  getServiceCategoryStatusQuantityUsedNCBucketThunkMiddleware,
...
const UsedNCBucketDetails = () => {
  ...
  const handleOpen = (row) => {
    ...
    const serviceCategory = path.searchQuary?.[0]?.serviceCategory;
    path.changeEndPoint(
      `used-nc-bucket-share-by-selecting?page=${1}&serviceCategory=${serviceCategory}&status=${
        row?.status
      }`
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <MyButton
          className="bg-green-700 py-2 px-3 flex"
  ...
  return (
    <div className="p-3">
      <div className="flex justify-start main-text items-center gap-x-3">
        <Heading text="Used NC Bucket Details" showHeading />
      </div>
...
export default UsedNCBucketDetails;
