...
import { getServiceCategoryStatusLeadsDataThunkMiddleware } from "../../redux/features/bucket";
import { TiArrowForwardOutline } from "react-icons/ti";

const NcBucketShareBySelecting = () => {
  const dispatch = useDispatch();
  const location = useLocation();
  const path = usePath();
  const navigate = useNavigate();
  const axiosInstance = useAxios();
  const { serviceCategoryStatusLeadsData } = useSelector(
    (state) => state.bucket
  );
  // console.log('serviceCategoryStatusLeadsData', serviceCategoryStatusLeadsData);

  const allLeads = useMemo(() => {
    return serviceCategoryStatusLeadsData?.allLead;
  }, [serviceCategoryStatusLeadsData]);

  const totalPages = useMemo(() => {
    // return serviceCategoryStatusLeadsData?.countAllLead;
    const countAllLead = serviceCategoryStatusLeadsData?.countAllLead;
  ...
  useEffect(() => {
    ...
    dispatch(getServiceCategoryStatusLeadsDataThunkMiddleware(payload));
...
export default NcBucketShareBySelecting;
