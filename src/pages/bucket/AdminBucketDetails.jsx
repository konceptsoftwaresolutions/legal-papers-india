...
import { getServiceCategoryStatusQuantityThunkMiddleware } from "../../redux/features/bucket";
...
const AdminBucketDetails = () => {
  ...
  const { serviceCategoryStatusQuantity } = useSelector(
    (state) => state.bucket
  );
  ...
  useEffect(() => {
    ...
    dispatch(getServiceCategoryStatusQuantityThunkMiddleware(serviceCategory));
  ...
  const handleOpen = (row) => {
    ...
    const serviceCategory = path.searchQuary?.[0]?.serviceCategory;
    path.changeEndPoint(
      `nc-bucket-share-by-selecting?page=${1}&serviceCategory=${serviceCategory}&status=${
        row?.status
      }`
    );
  };
    ...
    <div className="p-3">
      ...
      <div className="w-full py-4">
        <DataTable
          columns={columns}
          data={
            serviceCategoryStatusQuantity ? serviceCategoryStatusQuantity : []
          }
          pagination
...
export default AdminBucketDetails;
