...
const TotalNCBucketView = () => {
  const dispatch = useDispatch();
  const path = usePath();
  const navigate = useNavigate();
  const location = useLocation();
  const axiosInstance = useAxios();
  const { serviceCategoryStatusLeadsDataTotal } = useSelector(
    (state) => state.bucket
  );
  ...
  const allLeads = useMemo(() => {
    return serviceCategoryStatusLeadsDataTotal?.allLead;
  }, [serviceCategoryStatusLeadsDataTotal]);

  const totalPages = useMemo(() => {
    // return serviceCategoryStatusLeadsData?.countAllLead;
    const countAllLead = serviceCategoryStatusLeadsDataTotal?.countAllLead;
  ...
  useEffect(() => {
    ...
    dispatch(getServiceCategoryStatusLeadsDataTotalThunkMiddleware(payload));
  ...
  return (
    <>
      <LeadFilter
        ...
        filterFor="total-nc-bucket-view"
        endPoint="total-nc-bucket-view"
...
export default TotalNCBucketView;
