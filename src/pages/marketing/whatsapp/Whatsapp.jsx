...
import {
  ...
  getDropDownFieldsForVariables,
  getWATemplateById,
  whatsAppTemplateSaveSend,
} from "../../../redux/features/marketing";
import SampleMsgModal from "./SampleMsgModal";
import MarketingLeadFilter from "./MarketingLeadFilter";
...
const Whatsapp = () => {
  ...
  const { allWhatsAppTemplates, dropdownVar } = useSelector(
  ...
  useEffect(() => {
    if (!hasFetchedDropdownData.current) {
      dispatch(getAllWhatsAppTemplates());
      dispatch(getDropDownFieldsForVariables());
  ...
  useEffect(() => {
    if (selectedTemplateOption) {
      dispatch(
        getWATemplateById(selectedTemplateOption, (success, data) => {
  ...
  const handleCreateTemplate = () => {
    navigate(`/${role}/whatsapptemplate`);
  };
  ...
  const onSubmitHandler = (data) => {
    ...
    if (submitAction === "save") {
      ...
      dispatch(
        whatsAppTemplateSaveSend(payload, setSavesendLoading, (success) => {
          if (success) {
            reset();
            reset({
              leadId: "",
  ...
  return (
    <div className="p-5">
      <Heading text="Whatsapp" showHeading />
      ...
      <div className="p-3 bg-white shadow-lg mt-3 rounded-lg border-gray-600 flex justify-between items-center flex-col md:flex-row gap-y-2 md:gap-y-0">
        <p className="text-xl font-semibold">Send Whatsapp</p>
